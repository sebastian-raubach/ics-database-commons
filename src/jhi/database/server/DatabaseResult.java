...
public final class DatabaseResult
{
	...
	/**
	 * Retrieves the value of the designated column in the current row of this {@link DatabaseResult} object as a float in the Java programming
	 * language.
	 *
	 * @param columnLabel the label for the column specified with the SQL <code>AS</code> clause. If the SQL <code>AS</code> clause was not specified,
	 *                    then the label is the name of the column
	 * @return the column value; if the value is SQL <code>NULL</code>, the value returned is 0
	 * @throws DatabaseException Thrown if {@link ResultSet#getFloat(String)} fails
	 * @see ResultSet#getFloat(String)
	 */
	public Float getFloat(String columnLabel) throws DatabaseException
	{
		try
		{
			/* Check if this column exists in the result */
		...
		try
		{
			float result = this.rs.getFloat(columnLabel);
